% ─────────────────────────────────────────────────────────────
% SQL Race Example: Extract to Timetable
% ─────────────────────────────────────────────────────────────
%
% Demonstrates: Extracting parameter data and converting to a MATLAB timetable
...
% Output: MATLAB timetable with duration-based time index
%
...
try
    ...
    try
        ...
        % --- Extract to MATLAB arrays ---
        timestampsNs = zeros(nSamples, 1, 'int64');
        values = zeros(nSamples, 1);
        for i = 1:nSamples
            timestampsNs(i) = samples.Timestamp(i);
            values(i) = samples.Data(i);
        end

        % --- Convert to MATLAB timetable ---
        % Use duration from session start for the time axis
        startNs = timestampsNs(1);
        relativeSeconds = double(timestampsNs - startNs) / 1e9;
        timeVector = seconds(relativeSeconds);

        tt = timetable(timeVector, values, 'VariableNames', {paramId});

        fprintf('Timetable: %d rows\n', height(tt));
        disp(head(tt, 10));
