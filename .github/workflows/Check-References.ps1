    ...
    foreach($packageFile in $packageFiles) { 
        ...
        foreach($validRegex in $VersionMustContain){
            if($version -notmatch $validRegex) {
                ...
                Write-Error "Package file '$packageFile' is referencing an invalid build of '$packageName' - $version must contain $validRegex"
            }
        }
        ...
        foreach($invalidRegex in $VersionMustNotContain){
            if($version -match $invalidRegex) {
                ...
                Write-Error "Package file '$packageFile' is referencing an invalid build of '$packageName' - $version must not contain $invalidRegex"
            }
        }
    }
}
